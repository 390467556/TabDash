...
- **常访问页面** 展示最近 15 天访问最多的页面，支持 Pin 钉住常驻
...
- **4 种氛围背景** (Night / Dusk / Day / Aurora) — 灵感来自 Superwhisper 的深色渐变 + blend-mode 多层混合，一键循环切换
...
最近 15 天访问次数最多的 8 个页面以卡片网格展示在顶部。点击任意卡片打开该页面。

- 鼠标悬停在卡片上点击 **x** 可以隐藏，隐藏后自动补充新卡片
- 被隐藏的 URL 跨会话记忆
- URL 经过归一化处理，同一页面不会重复出现
- **Pin（钉住）**：悬停卡片点击图钉图标，该页面将常驻列表，不受访问频率影响
- **从打开的标签中 Pin**：在"Open Tabs"区域的页面条目上点击书签图标，可直接将该网页钉到常访问列表
- **Unpin（取消钉住）**：再次点击图钉图标，恢复为按频率动态调整
- 钉住的卡片排在最前面，剩余位置由历史频率动态填充

### 背景场景切换

点击右上角的 **画笔图标** 循环切换 4 种氛围背景：

| 场景 | 风格 |
|------|------|
| Night | 深黑底 + 柔和的海军蓝/紫色光晕（默认） |
| Dusk | 暗靛蓝 + 柔和的紫色氛围 |
| Day | 深海蓝 + 暖琥珀色调 |
| Aurora | 午夜蓝 + 冷钢蓝高光 |

偏好设置本地保存。切换时有 1.2 秒平滑过渡。
...
| 视觉风格 | 毛玻璃 + 多层渐变 + noise 纹理（Superwhisper 风格） |
