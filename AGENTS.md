# AGENTS.md -- TabDash: Setup & Onboarding Guide for Coding Agents

You're installing **TabDash** for the user. Your job is not just to set it up -- it's to get them excited about using it.
...
> **TabDash** replaces your new tab page with a clean dashboard of everything you have open, grouped by domain.
>
...
> - **Frequently visited** shows your top 10 most-visited pages for quick access
> - **Light / Dark mode** Apple-style frosted glass light theme, or industrial dark theme
...
>
> You should see "TabDash" appear in your extensions list.
...
> You're all set! Open a **new tab** and you'll see TabDash.
>
> Here's how it works:
> 1. **Your open tabs are grouped by domain** in a masonry grid layout.
> 2. **Homepages** (Gmail inbox, X home, YouTube, etc.) are in their own group at the top.
> 3. **Frequently visited** pages from the last 15 days appear at the top for quick access.
...
> 9. **Toggle light/dark mode** with the sun/moon button in the header.
>
...
- TabDash is a pure Chrome extension. No server, no Node.js, no npm.
- Saved tabs are stored in `chrome.storage.local` (persists across sessions).
- 100% local. No data is sent to any external service.
- To update: `git pull`, then reload the extension in `chrome://extensions`.
